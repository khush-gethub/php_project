...
<div class="container mx-auto p-4">
    ...
    <div class="card lg:card-side bg-base-100 shadow-xl mb-8">
        <figure><img src="<?php echo $user['avatar']; ?>" alt="Avatar" class="w-48 h-48 rounded-full"></figure>
        <div class="card-body">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        if ($user_posts_result->num_rows > 0) {
            while ($post = $user_posts_result->fetch_assoc()) {
                ...
                echo "<figure><img src='" . $post["image"] . "' alt='" . $post["title"] . "' /></figure>";
                echo "<div class='card-body'>";
                echo "<h2 class='card-title'>" . $post["title"] . "</h2>";
                echo "<p>" . substr($post["description"], 0, 100) . "...</p>";
                echo "<div class='card-actions justify-end'>";
                echo "<a href='post.php?id=" . $post["id"] . "' class='btn btn-info btn-sm'>View</a>";
                echo "<a href='edit_user_post.php?id=" . $post["id"] . "' class='btn btn-warning btn-sm'>Edit</a>";
                echo "<a href='delete_user_post.php?id=" . $post["id"] . "' class='btn btn-error btn-sm'>Delete</a>";
                echo "</div>";
                echo "</div>";
                echo "</div>";
            }
