...
if ($existing_reaction) {
    ...
    if ($existing_reaction['reaction_type'] == $reaction_type) {
        ...
        $sql = "UPDATE posts SET " . $reaction_type . "s = " . $reaction_type . "s - 1 WHERE id = ?";
        $stmt = $conn->prepare($sql);
        ...
        $sql = "UPDATE posts SET " . $reaction_type . "s = " . $reaction_type . "s + 1, " . $other_reaction . "s = " . $other_reaction . "s - 1 WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $post_id);
        $stmt->execute();
    }
